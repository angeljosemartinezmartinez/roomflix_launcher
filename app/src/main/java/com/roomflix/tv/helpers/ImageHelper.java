...
public class ImageHelper {
        ...
        if (!((Activity) context).isFinishing())
            Glide.with(context).load(url)
                    .apply(new RequestOptions()
                            .dontAnimate()
                            .diskCacheStrategy(DiskCacheStrategy.AUTOMATIC)) // Optimización: usar caché automático
                    .into(view);
    }
    ...
    public void loadRoundCorner(String url, ImageButton view, Context context) {
        if (context instanceof Activity) {
            Activity activity = (Activity) context;
            if (activity.isFinishing() || activity.isDestroyed()) {
                return; // Abortar carga si la actividad no es válida
            }
        }
        Glide.with(context).load(url)
                    .apply(new RequestOptions()
