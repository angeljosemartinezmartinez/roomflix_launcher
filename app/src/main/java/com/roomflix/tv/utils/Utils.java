...
public class Utils {
    ...
    /**
     * Crea un Context con el Locale aplicado (para usar en attachBaseContext)
     * Compatible con Android 7.0+ (API 24+) y versiones anteriores
     */
    public static Context createContextWithLocale(Context base, Locale locale) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
            ...
            // Método legacy para versiones anteriores a Android 7.0
            Locale.setDefault(locale);
            Configuration config = base.getResources().getConfiguration();
            config.locale = locale;
